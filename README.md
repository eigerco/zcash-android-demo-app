...
- Before you start using the features connected to the database, it's best to set a seed (`SEED`) in `Constants`, together with the wallet birthday (`WALLET_BIRTHDAY_HEIGHT`).
...
So, in general, for spending, this is how an ideal scenario goes:
 ...
 - Important: you will be able to spend the note only once, since additional synchronization requires more complexity. We believe this is an already good starting point.
